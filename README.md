...
| -------------------- | ------------: | ----------: | -------: | ------: |
| `speedy-router`      |   **95.5 kB** | **27.9 kB** | 104.5 kB | 29.5 kB |
| `speedy-router-core` |   **74.0 kB** | **21.6 kB** |  75.1 kB | 21.6 kB |

speedy-router is now smaller on both initial client graphs (0.95× gzip for `speedy-router`; core gzip 22086 vs 22152). Parallel route slots are tree-shaken out of this graph unless `createSlotRoute` is imported. The client load coordinator and SSR `load` chunk are dynamic imports and are not counted. The initial graph no longer includes TanStack's segment-tree matcher, hydrate, HMR refresh, or hash/memory history. Re-run with `pnpm size`.
