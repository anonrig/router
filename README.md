...
| -------------------- | ------------: | ------: | -----------: | ----------: |
| `speedy-router`      |      118.0 kB | 32.9 kB | **104.5 kB** | **29.5 kB** |
| `speedy-router-core` |       98.4 kB | 27.0 kB |  **75.1 kB** | **21.6 kB** |

TanStack is still smaller on both client graphs (1.12× gzip for `speedy-router`). Parallel route slots, `HeadContent` / `Scripts`, hash/memory history, and unused route hooks such as `useMatchRoute` are tree-shaken out of this graph unless `createSlotRoute` is imported. The client load coordinator and SSR `load` chunk are dynamic imports and are not counted. The remaining extra is the warm-path / matcher interners. The initial graph no longer includes TanStack's segment-tree matcher, hydrate, HMR refresh, or the lazy-route chunk loader. Re-run with `pnpm size`.
