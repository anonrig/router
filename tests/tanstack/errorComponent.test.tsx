...
import type { ErrorComponentProps, RouterHistory } from '@tanstack/react-router'
...
describe.each([true, false])('with lazy errorComponent=%s', (isUsingLazyError) => {
  ...
  describe('stale route errors do not leak after navigation', () => {
    test.each([
      ...
      async ({ routeOptions }) => {
        ...
        expect(await screen.findByText('Index route content')).toBeInTheDocument()
...
test('global catch boundary resets when a background child generation recovers', async () => {
  ...
  render(<RouterProvider router={router} />)
  expect(await screen.findByText('stale child render failed')).toBeInTheDocument()
  ...
  expect(await screen.findByText('Recovered child revision 2')).toBeInTheDocument()
  expect(screen.queryByText('stale child render failed')).not.toBeInTheDocument()
})
...
test('ancestor route errorComponent resets when a background child generation recovers', async () => {
  ...
  try {
    render(<RouterProvider router={router} />)
    expect(await screen.findByText('Ancestor error: stale child render failed')).toBeInTheDocument()

    invalidation = router.invalidate({
      filter: (match) => match.routeId === childRoute.id,
    })
    await vi.waitFor(() => expect(loaderCalls).toBe(2))
    expect(screen.getByText('Ancestor error: stale child render failed')).toBeInTheDocument()
    refresh.resolve(2)
    await invalidation
...
test('#4684: SSR renders head content when beforeLoad throws', async () => {
  ...
  expect(serverDocument.body.textContent).toContain('Error UI: beforeLoad failed')
  expect(serverDocument.head.querySelector('title')?.textContent).toBe('Error title')
  expect(
    serverDocument.head.querySelector('link[rel="stylesheet"][href="/global.css"]'),
  ).not.toBeNull()
})
...
describe('notFoundComponent is rendered when an error is thrown in params.parse', () => {
  test('displays notFoundComponent when error is thrown in params.parse', async () => {
    ...
    const routeTree = rootRoute.addChildren([indexRoute, restaurantRoute.addChildren([pizzaRoute])])
