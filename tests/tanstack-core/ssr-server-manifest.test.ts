...
import { GLOBAL_TSR } from '@tanstack/router-core/ssr/constants'
...
import type {
  ...
  ManifestRouteAssets,
  ServerManifest,
} from '@tanstack/router-core/manifest'
import type { DehydratedRouter } from '@tanstack/router-core/ssr/types'
...
describe('attachRouterServerSsrUtils manifest dehydration', () => {
  ...
  test('maps request-scoped preload links into SSR manifest data', async () => {
    ...
    expect(router.ssr!.manifest?.routes.__root__).toMatchObject({
      preloads: [{ href: '/assets/rsc-client.js', crossOrigin: 'anonymous' }, '/assets/root.js'],
      scripts: [
        {
          attrs: {
            src: '/assets/request-script.js',
  ...
  test('dehydrates request-scoped preload links into manifest data', async () => {
    ...
    expect((dehydratedManifest.routes.__root__?.css as any).links).toBe(undefined)
  })
  ...
  test('inlines stylesheet assets for SSR and strips stylesheet links from dehydration', async () => {
    ...
    const allLinks = Object.values(dehydratedManifest.routes).flatMap((route) => route.css ?? [])
    ...
    expect(dehydratedManifest.routes['/']?.preloads).toEqual(['/assets/index.js'])
  })
  ...
  test('strips only inlinable stylesheet links from dehydrated manifest data', async () => {
    ...
    expect(dehydratedManifest.routes['/']?.css).toEqual(['/assets/index-linked.css'])
    expect(dehydratedManifest.routes['/']?.preloads).toEqual(['/assets/index.js'])
  })
