...
function findHtmlBoundaryCombined(str: string): BoundaryScanResult {
  ...
  while (searchFrom >= MIN_CLOSING_TAG_LENGTH - 1) {
    ...
    if (tagNameStart < closeIndex) {
      const startCode = str.charCodeAt(tagNameStart)
      if (
        ((startCode >= 97 && startCode <= 122) || (startCode >= 65 && startCode <= 90)) &&
        j >= 1 &&
        str.charCodeAt(j) === 47 &&
...
function verifyBodyImplementations() {
  ...
  for (const testCase of testCases) {
    ...
    const nativeLowerResult = findBodyEndTagNativeLowerThenLastOpenSlash(testCase)
    ...
    const lazyEncodedBodyEndIndex = lazyEncodedResult < -1 ? -lazyEncodedResult - 2 : -1
    const lazyEncodedLastClosingTagEnd = lazyEncodedResult < -1 ? -1 : lazyEncodedResult
...
benchBodyEndTagDetection('Plain Text Body Tail (~16KB)', generateLargePlainTextBodyChunk())
benchBodyEndTagDetection('Large Chunk Without </body>', generateLargeChunkNoBody())

benchBoundaryDetection('Large Final Chunk With </body>', generateLargeChunk())
benchBoundaryDetection('Large Chunk Without </body>', generateLargeChunkNoBody())
