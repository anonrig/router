...
import { dehydrateSsrMatchId, hydrateSsrMatchId } from '@tanstack/router-core/ssr/ssr-match-id'
...
const deepIds = Array.from(
  { length: 100 },
  (_, index) => `${Array.from({ length: 32 }, (__, depth) => `/route-${depth}`).join('')}/${index}`,
)
const reservedIds = Array.from({ length: 100 }, (_, index) => `~/\0/\uFFFD/~0/~r/${index}`)
const normalizedDeepIds = deepIds.map((id) => dehydrateSsrMatchId(id).replaceAll('\0', '\uFFFD'))
let benchmarkSink = 0
