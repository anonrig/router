...
export function getScrollRestorationScriptForRouter(router: AnyRouter) {
  const scrollRestoration = router.options.scrollRestoration as
    | boolean
    | ((opts: { location: AnyRouter['latestLocation'] }) => boolean)
    | undefined
  if (
    typeof scrollRestoration === 'function' &&
    !scrollRestoration({ location: router.latestLocation })
  ) {
    return null
  }
